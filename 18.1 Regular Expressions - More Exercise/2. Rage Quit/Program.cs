...
namespace _02._Rage_Quit
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] symbols = Regex.Matches(input, symbolsPattern).Select(x => x.Value).ToArray();
            int[] digits = Regex.Matches(input, digitsPattern).Select(x => int.Parse(x.Value)).ToArray();
